...
class _VacanteDetalleScreenState extends State<VacanteDetalleScreen> {
  ...
  Future<void> _toggleGuardar() async {
    // Verificación explícita con mensaje claro
    if (_usuarioId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Inicia sesión para guardar vacantes.'),
  ...
  void _mostrarDialogoPostulacion() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Te vas a postular a:\n\n${_vacante!.titulo}\n${_vacante!.empresa ?? ''}'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancelar')),
          ElevatedButton(
            onPressed: () async { Navigator.pop(context); await _ejecutarPostulacion(); },
  ...
  Future<void> _ejecutarPostulacion() async {
    ...
    switch (vm.state) {
      ...
      case PostulacionState.yaPostulado:
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Ya te postulaste a esta vacante.'), backgroundColor: Colors.orange),
        );
        vm.resetState();
      case PostulacionState.error:
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(vm.errorMsg ?? 'Error'), backgroundColor: AppColors.error),
        );
        vm.resetState();
      default: break;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_vacante!.titulo, maxLines: 1, overflow: TextOverflow.ellipsis),
        actions: [
          _togglingGuard
              ? const Padding(
              ...
              : IconButton(
                  icon: AnimatedSwitcher(
                    ...
                    child: Icon(
                      ...
                      color: _guardada ? Colors.amber : Colors.white,
                    ),
                  ),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Descripción del cargo',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Text(_vacante!.descripcion ?? '',
                style: const TextStyle(fontSize: 15, height: 1.5)),
            const SizedBox(height: 20),
            const Text('Requisitos',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Text(_vacante!.requisitos ?? '',
                style: const TextStyle(fontSize: 15, height: 1.5)),
        ...
        child: Column(
          ...
          children: [
            ...
            _yaPostulado
                ...
                : ElevatedButton.icon(
                    onPressed: (!_vacante!.activa || cargandoPost) ? null : _mostrarDialogoPostulacion,
                    icon: cargandoPost
